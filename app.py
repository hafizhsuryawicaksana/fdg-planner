...
# 1. Tentukan "basedir" (path absolut ke folder proyek ini)
...
def calculate_initial_activity(final_activity, time_minutes):
    """Kalkulator peluruhan mundur A(0) = A(t) * e^(lambda * t)"""
    if time_minutes < 0:
        time_minutes = 0
...
def api_production_plan():
    """API untuk menghitung dan menyimpan data"""
    try:
        # --- 1. AMBIL INPUT ---
        final_activity = request.args.get('final_activity', type=float)
        injection_time_str = request.args.get('injection_time')
        ...
        new_run = ProductionRun(
            ...
            yield_synthesis=yield_synthesis_percent, # Simpan 50
            t_qc=t_qc,
...
# --- HANYA UNTUK RUN LOKAL (VS CODE) ---
if __name__ == '__main__':
    print(f"Database akan disimpan di: {app.config['SQLALCHEMY_DATABASE_URI']}")
