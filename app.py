...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:////var/data/fdg_planner.db'
...
class ProductionRun(db.Model):
    ...
    # Hasil Jadwal (BARU)
    deadline_dispatch = db.Column(db.String(8)) # <-- INI JUGA HILANG
    deadline_qc_finish = db.Column(db.String(8)) # <-- INI JUGA HILANG
    deadline_eos = db.Column(db.String(8)) # <-- INI JUGA HILANG
    deadline_eob = db.Column(db.String(8)) # <-- INI JUGA HILANG
...
def api_production_plan():
    try:
        ...
        yield_synthesis = request.args.get('yield_synthesis', type=float, default=0.5)
        ...
        A_eob_yield_corrected = A_post_synthesis / yield_synthesis
        ...
        new_run = ProductionRun(
            ...
            yield_synthesis=yield_synthesis,
            t_qc=t_qc,
...
if __name__ == '__main__':
    # (BARU) Trik untuk membuat file .db secara otomatis
